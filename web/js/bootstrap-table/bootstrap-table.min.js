...
return true;});return index;};var getFieldIndexFromColumnIndex=function(columns,fieldIndex){$.each(columns,function(i,column){if(!column.visible){fieldIndex--;}
if(i==fieldIndex){return false;}});return fieldIndex;};var setFieldIndex=function(columns){var i,j,k,totalCol=0,flag=[];for(i=0;i<columns[0].length;i++){totalCol+=columns[0][i].colspan||1;}
...
this.$header.find('tr').each(function(){var column=[];$(this).find('th').each(function(){if(typeof $(this).data('field')!=='undefined'){$(this).data('field',new String($(this).data('field')).valueOf());}
...
if(index!==-1){this.data.sort(function(a,b){if(that.header.sortNames[index]){name=that.header.sortNames[index];}
...
sprintf(' btn-%s',this.options.buttonsClass)+'" type="button" name="paginationSwitch" title="%s">',this.options.formatPaginationSwitch()),sprintf('<i class="%s %s"></i>',this.options.iconsPrefix,this.options.icons.paginationSwitchDown),'</button>');}
...
var s=this.searchText&&this.searchText.toLowerCase();var f=$.isEmptyObject(this.filterColumns)?null:this.filterColumns;this.data=f?$.grep(this.options.data,function(item,i){for(var key in f){if($.isArray(f[key])){if($.inArray(item[key],f[key])===-1){return false;}}else if(item[key]!==f[key]){return false;}}
